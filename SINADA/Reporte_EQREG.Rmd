...
SINADA1_REGISTRO_TAREAS <- read_excel("SINADA1_REGISTRO_DATA.xlsx", sheet = "TAREAS")
SINADA2_REGISTRO_METAS <- read_excel("SINADA1_REGISTRO_DATA.xlsx", sheet = "METAS")
SINADA2_REGISTRO_NOLABORADOS <- read_excel("SINADA1_REGISTRO_DATA.xlsx", sheet = "NOLABORADOS")
FERIADOS <- read_excel("FERIADOS.xlsx")

# III. SELECCIONANDO ESPECIALISTA----

LISTADO_ESPEC <-  SINADA2_REGISTRO_METAS %>% filter(ESTADO == 'ACTIVO')
...
datos_final1 <- SINADA1_REGISTRO_TAREAS %>% filter(ESPECIALISTA == espec)

metas_final1 <- SINADA2_REGISTRO_METAS %>% filter(DATOS_ACTUALES == "ACTUAL") %>% 
  filter(ESPECIALISTA == espec)

dias_no_trabajados <- SINADA2_REGISTRO_NOLABORADOS %>% filter(ESPECIALISTA == espec)
...
datos_final2 <- filter(datos_final1,datos_final1$FECHA_ACCION >= INICIOFECHA & datos_final1$FECHA_ACCION <= FINFECHA) %>% arrange(FECHA_ACCION)
Cantidad_productos_rpta_eq1 <- data.frame(table(datos_final2$ESPECIALISTA))


datos_final3 <- filter(datos_final1,datos_final1$FECHA_CARGA >= INICIOFECHA & datos_final1$FECHA_CARGA <= FINFECHA) %>% arrange(FECHA_CARGA)
Cantidad2_productos_rpta_eq1 <- data.frame(table(datos_final3$ESPECIALISTA))
...
Produccion1eq1 <- data.frame(
  "No." = seq(1:nrow(datos_final2)),
  "Recepción" = datos_final2$CODIGO_RECEPCION,
  "Medio" = datos_final2$MEDIO_RECEPCION,
  "Fecha acción" = as.character(datos_final2$FECHA_ACCION, format="%d/%m/%Y"),
  "Producto" = datos_final2$PRODUCTO_ACCION,
  "HT" = datos_final2$HOJA_TRAMITE_PRODUCTO
)

Produccion2eq1 <- data.frame(
  "No." = seq(1:nrow(datos_final3)),
  "Recepción" = datos_final3$CODIGO_RECEPCION,
  "Medio" = datos_final3$MEDIO_RECEPCION,
  "Fecha envío" = as.character(datos_final3$FECHA_CARGA, format="%d/%m/%Y"),
  "Producto" = datos_final3$PRODUCTO_ACCION,
  "HT" = datos_final3$HOJA_TRAMITE_PRODUCTO
)
...
RESULTADO_META2 <- round((Cantidad2_productos_rpta_eq1$Freq / (Meta_diaria1$META_DIARIA_IDEAL_SELECCIONADA*(Dias_laborables))), 2) * 100
...
kable(Meta_rptas_eq1, caption = "Porcentaje de calificación de comunicaciones ingresadas como potenciales denuncias ambientales de acuerdo al MAPRO", col.names = gsub("[.]", " ", names(Meta_rptas_eq1)), format = "latex", align = "ccc") %>%
...
kable(Meta2_rptas_eq1, caption = "Porcentaje de documentos emitidos en plazo, derivados de la calificación de comunicaciones ingresadas como potenciales denuncias ambientales de acuerdo a lo establecido en el MAPRO", col.names = gsub("[.]", " ", names(Meta_rptas_eq1)), format = "latex", align = "ccc") %>%
kable_styling(font_size = 10, bootstrap_options = "bordered", latex_options = "HOLD_position", full_width = FALSE) %>%
column_spec(1 , latex_valign = "m", width = "5cm", border_left = TRUE) %>%
column_spec(2 , latex_valign = "m", width = "5cm") %>%
column_spec(3 , latex_valign = "m", width = "5cm", border_right = TRUE) %>%
