...
# I.2 Determinar el periodo comprendido en el informe ----
som <- function(x) {
...
# II. Base de datos consolidado ----
COFEMA_ENTREGABLES <- read_xlsx("")
# Directorio donde se guaradan los documentos generados
dir <- ""
...
Este reporte[^1] contiene las metas correspondientes al equipo de Coordinación de Fiscalías Especializadas en Materia Ambiental en el periodo comprendido entre el **`r day(Inicio_periodo)` de `r Mes_inicio` al `r day(Fin)` de `r Mes_fin` de `r year(now())`**:
...
ENTREGABLE <- data.frame(
 "No." = seq(1:nrow(datos_final)),
  ...
  "Fecha_aprobación" =  as.character(datos_final$F_APROB, format="%d/%m/%Y")
)
...
datos_meta1 = datos_final[!is.na(datos_final$TIEMPO_RESP), ]
...
datos_meta2 = datos_final[!is.na(datos_final$TIEMPO_RESP), ]
datos_aprob = datos_meta2[!is.na(datos_meta2$F_APROB), ]
n_meta2 <- nrow(datos_meta2)
n_aprob <- nrow(datos_aprob)

PORCENTAJE_CUMPLIDO2 <- n_aprob/n_meta2
...
write.xlsx(ENTREGABLE,file = file.path(carpeta, n_archivo), sheetName="Data")
